...
const admin = require('firebase-admin');
  var firebaseConfig = {
    ...
    storageBucket: "database-16835.appspot.com",
    messagingSenderId: "712977668377",
    appId: "1:712977668377:web:b412fa742e33aef3"
  };
...
//set token first 
 ...
 exports.createStripeCharge = functions.database.ref('stripe_customers/{userId}/charges/{id}').onCreate(async (snap, context) => {
      ...
      try {
        ...
        const snapshot = await admin.database().ref('stripe_customers/${context.params.userId}/customer_id').get()
...
// Add a payment source (card) for a user by writing a stripe payment source token to Realtime database
 exports.addPaymentSource = functions.database.ref('/stripe_customers/{userId}/sources/{pushId}/token').onCreate(async (snap, context) => {
  ...
  try {
    ...
    return snap.ref.parent.set(response, {merge: true});
  } catch (error) {
    await snap.ref.set({'error':userFacingMessage(error)},{merge:true});
    return reportError(error, {user: context.params.userId});
  }
...
// When a user deletes their account, clean up after them
 exports.cleanupUser = functions.auth.user().onDelete(async (user) => {
  const snapshot = await admin.database().ref('stripe_customers/${user.uid}').get();
